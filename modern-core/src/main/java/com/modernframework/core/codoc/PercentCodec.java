...
import com.modernframework.core.utils.HexUtils;
...
public class PercentCodec implements Serializable {
	...
	public static PercentCodec of(CharSequence chars) {
		Asserts.notNull(chars, "chars must not be null");
	...
	public String encode(CharSequence path, Charset charset, char... customSafeChar) {
		...
		for (int i = 0; i < path.length(); i++) {
			c = path.charAt(i);
			if (safeCharacters.get(c) || ArrayUtils.contains(customSafeChar, c)) {
				rewrittenPath.append(c);
			} else if (encodeSpaceAsPlus && c == CharConstant.SPACE) {
				// 对于空格单独处理
