...
import com.modernframework.core.utils.ObjectUtils;
...
public class ReferenceConcurrentMap<K, V> implements ConcurrentMap<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
     ...
     * @return 值对象
     */
    public V computeIfAbsent(K key, ThrowableSupplier<? extends V> supplier) {
        return computeIfAbsent(key, (keyParam) -> supplier.getWithRe());
    }
    ...
    @Override
    public Set<K> keySet() {
        return this.raw.keySet().stream()
                .map((reference) -> null == reference ? null : reference.get())
                .collect(Collectors.toSet());
        
//        非高效方式的set转换，应该返回一个view
    ...
    private static class WeakKey<K> extends WeakReference<K> {
        ...
        public boolean equals(Object other) {
            if (other == this) {
                ...
                return ObjectUtils.equals(((WeakKey<?>) other).get(), get());
            }
            return false;
        }
    }
    ...
    private static class SoftKey<K> extends SoftReference<K> {
        ...
        public boolean equals(Object other) {
            if (other == this) {
                ...
                return ObjectUtils.equals(((SoftKey<?>) other).get(), get());
            }
            return false;
        }
    }
