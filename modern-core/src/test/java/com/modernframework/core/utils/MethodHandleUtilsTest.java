...
public class MethodHandleUtilsTest {
    ...
    public void invokeDefaultTest() {
        Duck duck = (Duck) Proxy.newProxyInstance(
                ...
                MethodHandleUtils::invokeSpecial);

        Assert.assertEquals("Quack", duck.quack());

        // 测试子类执行default方法
        final Method quackMethod = ReflectUtils.getMethod(Duck.class, "quack");
        String quack = MethodHandleUtils.invokeSpecial(new BigDuck(), quackMethod);
        Assert.assertEquals("Quack", quack);
        Assert.assertEquals("Quack", MethodHandleUtils.invoke(new BigDuck(), quackMethod));

        final Method getDuckMethod = ReflectUtils.getMethod(Duck.class, "getDuck", int.class);
        String getDuck = MethodHandleUtils.invoke(null, getDuckMethod, 12);
        Assert.assertEquals("Duck 12", getDuck);

        // 测试反射执行默认方法
        quack = ReflectUtils.invoke(new Duck() {}, quackMethod);
        Assert.assertEquals("Quack", quack);
    }
    ...
    public void invokeTest() {
        // 测试执行普通方法
        final int size = MethodHandleUtils.invokeSpecial(new BigDuck(),
                ReflectUtils.getMethod(BigDuck.class, "getSize"));
    ...
    public void invokeStaticTest() {
        // 测试执行普通方法
        final String result = MethodHandleUtils.invoke(null,
                ReflectUtils.getMethod(Duck.class, "getDuck", int.class), 78);
        Assert.assertEquals("Duck 78", result);
    }

    @Test
    public void findMethodTest() throws Throwable {
        MethodHandle handle = MethodHandleUtils.findMethod(Duck.class, "quack",
        ...
        // 对象的方法获取
        handle = MethodHandleUtils.findMethod(BigDuck.class, "getSize",
    ...
    public void findStaticMethodTest() throws Throwable {
        final MethodHandle handle = MethodHandleUtils.findMethod(Duck.class, "getDuck",
    ...
    public void findPrivateMethodTest() throws Throwable {
        final MethodHandle handle = MethodHandleUtils.findMethod(BigDuck.class, "getPrivateValue",
                MethodType.methodType(String.class));
        Assert.assertNotNull(handle);
    ...
    public void findSuperMethodTest() throws Throwable {
        // 查找父类的方法
        final MethodHandle handle = MethodHandleUtils.findMethod(BigDuck.class, "quack",
                MethodType.methodType(String.class));
        Assert.assertNotNull(handle);
    ...
    public void findPrivateStaticMethodTest() throws Throwable {
        final MethodHandle handle = MethodHandleUtils.findMethod(BigDuck.class, "getPrivateStaticValue",
                MethodType.methodType(String.class));
        Assert.assertNotNull(handle);
