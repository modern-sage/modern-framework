...
public abstract class StatementUtil {
    ...
    private final Logger log = LoggerFactory.getLogger(StatementUtil.class);
    ...
    public static PreparedStatement fillParams(PreparedStatement ps, Object... params) {
        ...
        return fillParams(ps, Arrays.asList(params));
    }
     ...
     * @return {@link PreparedStatement}
     */
    public static PreparedStatement fillParams(PreparedStatement ps, Iterable<?> params) {
     ...
     */
    public static PreparedStatement fillParams(PreparedStatement ps, Iterable<?> params, Map<Integer, Integer> nullTypeCache) {
        if (null == params) {
            return ps;// 无参数
        }
    ...
    public static PreparedStatement prepareStatement(Connection conn, String sql, Object... params) {
        Asserts.notBlank(sql, "Sql String must be not blank!");

        sql = sql.trim();
        if (ArrayUtils.isEmpty(params)) {
            log.debug("SQL: {}", sql);
        } else {
            log.debug("SQL: {}, PARAMS: {}", sql, ArrayUtils.isEmpty(params) ? null : params);
        }
        PreparedStatement ps;
        try {
            if (StringUtils.startWithIgnoreCase(sql, "insert")) {
                // 插入默认返回主键
    ...
    public static PreparedStatement prepareStatementForBatch(Connection conn, String sql, Object[]... paramsBatch) throws SQLException {
        return prepareStatementForBatch(conn, sql, Arrays.asList(paramsBatch));
    }
     ...
     * @param dataMaps "?"对应参数批次列表
     * @return {@link PreparedStatement}
     * @throws SQLException SQL异常
     */
    public static PreparedStatement prepareStatementForBatch(Connection conn, String sql, List<String> fields, Map<String, Object>... dataMaps) throws SQLException {
        Asserts.notBlank(sql, "Sql String must be not blank!");
        ...
        for (Map<String, Object> entity : dataMaps) {
            StatementUtil.fillParams(ps, fields.stream().map(entity::get).collect(Collectors.toList()), nullTypeMap);
            ps.addBatch();
        }
        return ps;
    }
     ...
     * @param param      参数
     */
    public static void setParam(PreparedStatement ps, int paramIndex, Object param) {
     ...
     */
    private static void setParam(PreparedStatement ps, int paramIndex, Object param, Map<Integer, Integer> nullTypeCache) {
        try {
            if (null == param) {
