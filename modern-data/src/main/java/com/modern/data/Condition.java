...
import com.modern.data.utils.SqlUtil;
...
public class Condition implements Cloneable {
    ...
    public String toString(List<Object> paramValues) {
        ...
        conditionStrBuilder.append(this.field).append(StrConstant.SPACE).append(this.operator);
    ...
    private void buildValuePartForBETWEEN(StringBuilder conditionStrBuilder, List<Object> paramValues) {
        ...
        if (isPlaceHolder()) {
            ...
            // 直接使用条件值
            conditionStrBuilder.append(StrConstant.SPACE).append(this.value);
        }

        // 处理 AND y
        conditionStrBuilder.append(StrConstant.SPACE).append(Link.AND);
        if (isPlaceHolder()) {
            // 使用条件表达式占位符
            conditionStrBuilder.append(" ?");
            if (null != paramValues) {
            ...
            // 直接使用条件值
            conditionStrBuilder.append(StrConstant.SPACE).append(this.secondValue);
        }
    }
    ...
    private void buildValuePartForIN(StringBuilder conditionStrBuilder, List<Object> paramValues) {
        ...
        List<?> valuesForIn = new LinkedList<>();
        // 占位符对应值列表
        if (value instanceof String) {
            valuesForIn = Arrays.asList(StringUtils.split((String) value, StrConstant.COMMA));
        } else if (value instanceof String[]) {
            valuesForIn = Arrays.asList((String[]) value);
        } else if (value instanceof Collection<?>) {
            valuesForIn = (List<?>) value;
        } else if (value != null) {
            valuesForIn = Collections.singletonList(String.valueOf(value));
        }

        if (isPlaceHolder()) {
            if(CollectionUtils.isNotEmpty(valuesForIn)) {
                conditionStrBuilder.append(valuesForIn.stream().map(x -> "?").collect(Collectors.joining(StrConstant.COMMA)));
            }
            if (null != paramValues) {
                paramValues.addAll(valuesForIn);
            }
        } else {
            conditionStrBuilder.append(valuesForIn.stream().map(String::valueOf).collect(Collectors.joining(StrConstant.COMMA)));
        }
        conditionStrBuilder.append(')');
    }
    ...
    private void parseValue() {
        ...
        // 对数组和集合值按照 IN 处理
        if (this.value instanceof Collection || ArrayUtils.isArray(this.value)) {
        ...
        // 处理null
        if (StringUtils.endWithIgnoreCase(valueStr, "null")) {
            if (StringUtils.equalsIgnoreCase("= null", valueStr) || StringUtils.equalsIgnoreCase("is null", valueStr)) {
                ...
                return;
            } else if (StringUtils.equalsIgnoreCase("!= null", valueStr) || StringUtils.equalsIgnoreCase("is not null", valueStr)) {
        ...
        List<String> strs = StringUtils.split(valueStr, StrConstant.SPACE, 2, true, true);
        if (strs.size() < 2) {
            return;
        }
        ...
        if (OPERATOR_BETWEEN.equals(firstPart)) {
            final List<String> betweenValueStrs = Arrays.stream(strs.get(1).split(Link.AND.toString())).map(String::trim).collect(Collectors.toList());
